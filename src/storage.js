...
export async function updateChannelSoundCloudPlaylist(file, channelId, syncResult) {
  if (syncResult.skipped || !syncResult.playlistId) return null;
  const library = await loadLibrary(file);
  const playlist = library.channels[channelId];
  if (!playlist) return null;
  playlist.soundCloudPlaylistId = syncResult.playlistId;
  playlist.soundCloudPlaylistUrl = syncResult.playlistUrl;
  playlist.soundCloudSyncedAt = new Date().toISOString();
  await saveLibrary(file, library);
  return playlist;
}
