...
import { addTrackToChannel } from './storage.js';
...
export function createDiscordBot({ token, dataFile, soundCloudClientId, soundCloudOAuthToken }) {
let heartbeatTimer;
let sequence = null;
...
await handleDiscordMessage(payload.d, { dataFile, soundCloudClientId, soundCloudOAuthToken });
});
...
export async function handleDiscordMessage(message, { dataFile, soundCloudClientId, soundCloudOAuthToken }) {
...
if (result.added) await syncPlaylistToSoundCloud(result.playlist, soundCloudOAuthToken);
} catch (error) {
